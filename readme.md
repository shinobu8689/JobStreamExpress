...
求人情報を自動収集し、ローカルLLMを使ってあなたのプロフィールとマッチングを行い、カバーレターと面接アドバイスを生成する自動求職パイプラインです。
...
1. [Ollama](https://ollama.com) をインストールし、モデルをダウンロードします（デフォルト: `gemma4:12b`）。
...
SeekのURL検索結果をクロールし、すべての求人をLLMパイプラインで処理します。
